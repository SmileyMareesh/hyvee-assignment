        ...
        if (canvas) {
            ...
            if (c) {
                ...
                const putPixel = (x: number, y: number, brightness: number) => {
                    ...
                    c.fillRect(x, y, 3, 3); // Increase size
                };

                const drawStar = (x: number, y: number, radius: number, colors: string[]) => {
                    const randomColor = colors[Math.floor(Math.random() * colors.length)];
                
                    c.beginPath();
                    for (let i = 0; i < 5; i++) {
                        c.lineTo(Math.cos((18 + i * 72) / 180 * Math.PI) * radius + x,
                                 -Math.sin((18 + i * 72) / 180 * Math.PI) * radius + y);
                        c.lineTo(Math.cos((54 + i * 72) / 180 * Math.PI) * radius / 2 + x,
                                 -Math.sin((54 + i * 72) / 180 * Math.PI) * radius / 2 + y);
                    }
                    c.closePath();
                    c.fillStyle = randomColor; // Use randomly selected color
                    c.fill();
                };
                ...
                const tick = (time: number) => {
                    ...
                    for (var i = 0; i < count; i++) {
                        ...
                        const b = 1 - d * d;
                        if(modalIsOpen){
                            drawStar(x, y, 2, ['#FF0000', '#00FF00', '#0000FF'])
                        }else{
                            putPixel(x, y, b);
                        }
                    }
